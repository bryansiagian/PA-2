...
<style>
    ...
    .category-pill { cursor: pointer; transition: 0.3s; border: 1px solid #ddd; color: #666; font-size: 13px; padding: 6px 18px; border-radius: 30px; background: #fff; display: inline-block; margin-right: 5px; margin-bottom: 10px; }
        ...
        <div class="row align-items-center">
            ...
            <div class="col-lg-6 mt-3 mt-lg-0">
                <div class="input-group shadow-sm rounded-pill overflow-hidden">
                    ...
                    <input type="text" id="newsSearch" class="form-control border-0 py-3" placeholder="Cari judul berita atau kegiatan..." onkeyup="filterPosts()">
                </div>
            </div>
        </div>
    </div>
...
<div class="container mt-4 mb-5">
    ...
    <div id="postsGrid" class="row g-4">
        ...
        <div class="col-12 text-center py-5">
            ...
            <p class="mt-2 text-muted">Memuat kabar terbaru...</p>
        </div>
    </div>
</div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content shadow-lg">
            ...
            <div class="modal-body p-4 p-md-5 pt-0">
                <img id="modalPostImg" src="" alt="Thumbnail">
                <div class="post-meta">
                    <span><i class="bi bi-calendar3 me-1"></i> <span id="modalPostDate"></span></span>
                    <span><i class="bi bi-tag me-1"></i> <span id="modalPostCategory" class="text-primary fw-bold"></span></span>
                    <span><i class="bi bi-person me-1"></i> <span id="modalPostAuthor">Admin</span></span>
                </div>
                <h2 id="modalPostTitle" class="fw-bold text-dark mb-4">Judul Berita</h2>
                <div id="modalPostContent" class="post-content-full">
                    <!-- Isi Lengkap -->
                </div>
            </div>
...
<script>
    ...
    function fetchPostCategories() {
        ...
        }).catch(err => console.error("Gagal muat kategori publik"));
    }

    function fetchAllPosts() {
        // Gunakan rute publik
        axios.get('/api/public/posts').then(res => {
            // Filter yang published
            allPosts = res.data.filter(p => p.status === 1);
            renderPosts(allPosts);
        }).catch(err => {
            document.getElementById('postsGrid').innerHTML = '<p class="text-center">Silakan refresh halaman.</p>';
        });
    }
    ...
    function renderPosts(data) {
        ...
        data.forEach(p => {
            ...
            const categoryName = p.category?.name || 'Berita';
                ...
                <div class="post-card d-flex flex-column h-100">
                    ...
                    <div class="p-4 d-flex flex-column flex-grow-1">
                        <div class="post-meta mb-2" style="font-size: 11px;">
                            ...
                            <span class="text-primary fw-bold">${categoryName}</span>
                        </div>
    ...
    function openPostDetail(id) {
        ...
        document.getElementById('modalPostAuthor').innerText = p.author?.name || 'Admin';
